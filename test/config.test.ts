...
describe("DEFAULT_FORMATS", () => {
  it("covers the language families from the original plugin", () => {
    expect(DEFAULT_FORMATS.python).toEqual({
      prefix: "#",
      fill: "#",
      suffix: "#",
    });
    expect(DEFAULT_FORMATS.typescript).toEqual({
      prefix: "//",
      fill: "/",
      suffix: "//",
    });
    expect(DEFAULT_FORMATS.css).toEqual({
      prefix: "/*",
      fill: "*",
      suffix: "*/",
    });
    expect(DEFAULT_FORMATS.sql).toEqual({
      prefix: "--",
      fill: "-",
      suffix: "--",
    });
    expect(DEFAULT_FORMATS.latex).toEqual({
      prefix: "%",
      fill: "%",
      suffix: "%",
    });
    expect(DEFAULT_FORMATS.markdown).toEqual({
      prefix: "<!--",
      fill: "-",
      suffix: "-->",
    });
...
describe("resolveFormat", () => {
  ...
  it("falls back to the built-in when an override is malformed", () => {
    expect(resolveFormat("python", { python: { prefix: "#" } })).toEqual(
      DEFAULT_FORMATS.python,
    );
    expect(
      resolveFormat("python", {
        python: { prefix: "#", fill: "##", suffix: "#" },
      }),
    ).toEqual(DEFAULT_FORMATS.python);
    expect(resolveFormat("python", { python: "nonsense" })).toEqual(
      DEFAULT_FORMATS.python,
    );
  });
  ...
  it("returns undefined when a malformed override has no built-in to fall back to", () => {
    expect(
      resolveFormat("clojure", { clojure: { fill: ";" } }),
    ).toBeUndefined();
  });
});
...
describe("resolveWidth", () => {
  ...
  it("accepts the { column, color } ruler form", () => {
    expect(
      resolveWidth([{ column: 100, color: "#333" }, { column: 72 }], undefined),
    ).toBe(72);
  });
  ...
  it("ignores malformed ruler entries", () => {
    expect(
      resolveWidth(["nonsense", null, { color: "#333" }, 90], undefined),
    ).toBe(90);
