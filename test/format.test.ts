...
describe("render", () => {
  ...
  it("accounts for multi-character prefixes and suffixes", () => {
    expect(
      render({ ...base, format: SLASH, title: "x", level: 1 })[0].length,
    ).toBe(79);
    expect(
      render({ ...base, format: BLOCK, title: "x", level: 1 })[0].length,
    ).toBe(79);
    expect(
      render({ ...base, format: HTML, title: "x", level: 1 })[0].length,
    ).toBe(79);
