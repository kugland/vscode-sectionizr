...
describe('sectionizr', () => {
  before(async () => {
    ...
    assert.ok(extension, 'expected the kugland.sectionizr extension to be discoverable');
  ...
  it('renders a plain divider for a blank line, not a banner with an empty title', async () => {
    ...
    assert.ok(/^#+$/.test(contentLines[0].trim()), JSON.stringify(contentLines));
  });
  ...
  it('keeps the title case when sectionizr.uppercaseLevel0 is false', async () => {
    ...
    try {
      ...
      const lines = editor.document.getText().split('\n');
      assert.ok(lines.some((line) => line.includes('My Section')), JSON.stringify(lines));
      assert.ok(!lines.some((line) => line.includes('MY SECTION')), JSON.stringify(lines));
    } finally {
      await vscode.workspace
        .getConfiguration('sectionizr', scope)
        .update('uppercaseLevel0', undefined, vscode.ConfigurationTarget.Global);
    }
  });

  it('collapses a multi-line selection into one banner using every non-blank line', async () => {
    const editor = await openPythonDocument('first line\nsecond line\nthird line\n');
    await vscode.workspace
      .getConfiguration('editor', editor.document.uri)
      .update('rulers', [80], vscode.ConfigurationTarget.Global);
  ...
  it('produces a sane banner when editor.rulers is unset, via the default-width fallback', async () => {
    ...
    await editorConfig.update('rulers', undefined, vscode.ConfigurationTarget.Global);
    ...
    try {
      ...
      await editorConfig.update('rulers', [80], vscode.ConfigurationTarget.Global);
    }
  });
});
